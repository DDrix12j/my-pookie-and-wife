(function(window){
    ...
    Seed.prototype = {
        ...
        drawText: function() {
            ...
            ctx.fillText("click me pookie", 30, -5);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
    ...
    function startHeartTree() {
        // This is the same as the code that runs on page load in happybday-main
        if (window.initLove) {
            window.initLove();
        }
    }

    window.initLove = function() {
        var canvas = $('#canvas');
        ...
        var textAnimate = eval(Jscex.compile("async", function () {
            $("#tree-text").show().typewriter({
                callback: function() {
                    // Hide the close button
                    $("#close-heart-tree").hide();
                    // Show the history button
                    $("#see-history-btn").show();
                }
            });
        }));
        ...
        var runAsync = eval(Jscex.compile("async", function () {
            ...
            $await(flowAnimate());
            $await(moveAnimate());
            textAnimate().start(); // <-- Only show text after all animations
        }));
